...
## 仅保留 M0 + (mt / subunit)
df <- df[df$model=="M0" & df$role %in% c("mt","subunit"), , drop=FALSE]

## ===== 规范命名 & 判定 nu-core =====
df$GENE_UP    <- toupper(df$gene)
df$COMPLEX_UP <- toupper(gsub("\\s+", "", df$complex))

has_suffix <- function(x, pat) grepl(paste0("^(", pat, ")[A-Z]?$"), x)

# CI 核心（NDUFS1-3/7/8；NDUFV1-2）
is_core_CI <- with(df, role=="subunit" & COMPLEX_UP=="CI" &
  ( has_suffix(GENE_UP, "NDUFS1|NDUFS2|NDUFS3|NDUFS7|NDUFS8") |
    has_suffix(GENE_UP, "NDUFV1|NDUFV2") ))

# CII 核心（SDHA-D）
is_core_CII <- with(df, role=="subunit" & COMPLEX_UP=="CII" &
  has_suffix(GENE_UP, "SDHA|SDHB|SDHC|SDHD"))

# CV 核心（兼容新旧命名）
# F1: ATP5A1/B/C1/D  <-> ATP5F1A/B/C/D
core_CV_F1 <- with(df, role=="subunit" & COMPLEX_UP=="CV" &
  ( has_suffix(GENE_UP, "ATP5A1|ATP5B|ATP5C1|ATP5D") |
    has_suffix(GENE_UP, "ATP5F1A|ATP5F1B|ATP5F1C|ATP5F1D") ))

# c 亚基：ATP5G1/2/3 <-> ATP5MC1/2/3
core_CV_c <- with(df, role=="subunit" & COMPLEX_UP=="CV" &
  has_suffix(GENE_UP, "ATP5G1|ATP5G2|ATP5G3|ATP5MC1|ATP5MC2|ATP5MC3"))

is_nu_core <- is_core_CI | is_core_CII | core_CV_F1 | core_CV_c

## 三组：mt / nu_core / nu_noncore
df$grp <- ifelse(df$role=="mt", "mt",
                 ifelse(is_nu_core, "nu_core", "nu_noncore"))
df <- df[!is.na(df$grp), , drop=FALSE]
df$grp <- factor(df$grp, levels=c("mt","nu_core","nu_noncore"))

## 调试：看看识别数
message("[counts]"); print(table(df$grp))

## ===== 样式 & 工具 =====
label_map <- c(mt="mtOXPHOS", nu_core="nu OXPHOS (core)", nu_noncore="nu OXPHOS (noncore)")
fill_cols <- c(mt="#8CC6EC",  nu_core="#F5A6A6",           nu_noncore="#E5C16C")
...
pairwise_pvals <- function(df_plot, yvar){
  lv  <- levels(df_plot$grp)
  prs <- combn(lv, 2, simplify = FALSE)
  raw_p <- sapply(prs, function(pr){
    x <- df_plot[df_plot$grp==pr[1], yvar, drop=TRUE]
    y <- df_plot[df_plot$grp==pr[2], yvar, drop=TRUE]
    if (length(x)<1 || length(y)<1) return(NA_real_)
    tryCatch(wilcox.test(x, y)$p.value, error=function(e) NA_real_)
  })
  adj_p <- p.adjust(raw_p, method="BH")
  ...
  ann   <- vapply(adj_p, function(p) ifelse(is.na(p),"p=NA", ifelse(p<0.001,"p<0.001", sprintf("p=%.3f",p))), "")
  data.frame(
    x1=vapply(comps, `[`, "", 1), x2=vapply(comps, `[`, "", 2),
    annotation=ann, p_adj=adj_p, stringsAsFactors=FALSE
  )
}
plot_three <- function(metric, ylab, out_stub, filter_for_omega=FALSE){
  dd <- df
  if (filter_for_omega) dd <- dd[!is.na(dd$omega) & dd$omega<999 & !is.na(dd$dS) & dd$dS>=min_dS_tree, ]
  dd <- dd[!is.na(dd[[metric]]), , drop=FALSE]; stopifnot(nrow(dd)>0)

  pw <- pairwise_pvals(dd, metric)
  if (signif_only) pw <- subset(pw, !is.na(p_adj) & p_adj<alpha)

  rng <- range(dd[[metric]], na.rm=TRUE); spn <- diff(rng); if(!is.finite(spn) || spn==0) spn <- 1
  top0 <- rng[2] + 0.25*spn; step <- 0.12*spn
  y_pos <- if (nrow(pw)) top0 + step*seq_len(nrow(pw)) else numeric(0)
  ...
  if (nrow(pw)) {
    comps <- Map(function(a,b) c(a,b), pw$x1, pw$x2)
    g <- g + ggsignif::geom_signif(comparisons=comps, annotations=pw$annotation,
                                   y_position=y_pos, tip_length=0.01, textsize=4.2,
                                   vjust=0.3, size=0.5) +
      coord_cartesian(ylim=c(rng[1]-0.15*spn, max(y_pos)+0.2*spn), clip="off")
  } else {
    kw <- tryCatch(kruskal.test(dd[[metric]] ~ dd$grp)$p.value, error=function(e) NA_real_)
    txt <- ifelse(is.na(kw),"", ifelse(kw<0.001,"p<0.001", sprintf("p=%.3f",kw)))
    g <- g + annotate("text", x=2, y=rng[2]+0.35*spn, label=txt, size=5) +
      coord_cartesian(ylim=c(rng[1]-0.15*spn, rng[2]+0.55*spn), clip="off")
  }
...
## ===== 出图 =====
plot_three("dN",    expression(d[N]),       "M0_dN_mt_nuCore_nuNoncore",   FALSE)
plot_three("dS",    expression(d[S]),       "M0_dS_mt_nuCore_nuNoncore",   FALSE)
plot_three("omega", expression(d[N]/d[S]),  "M0_dNdS_mt_nuCore_nuNoncore", TRUE)
