...
BASE_DIR <- paste0(
  "/work/cyu/gene_fst_work_withNorway/",
...
status_colors <- c(
  "AK top 5% only" = "#3B6FB6",
  "BC top 5% only" = "#2CA25F",
  "Top 5% in both" = "#8E44AD",
  "Background" = "grey80"
)
...
cutoff_AK <- quantile(
  compare_dt$mean_PBS_AK,
  0.95,
  na.rm = TRUE
)

cutoff_BC <- quantile(
  compare_dt$mean_PBS_BC,
  0.95,
  na.rm = TRUE
)
...
compare_dt[
  mean_PBS_AK > cutoff_AK &
  mean_PBS_BC > cutoff_BC,
  status := "Top 5% in both"
]

compare_dt[
  mean_PBS_AK > cutoff_AK &
  mean_PBS_BC <= cutoff_BC,
  status := "AK top 5% only"
]

compare_dt[
  mean_PBS_AK <= cutoff_AK &
  mean_PBS_BC > cutoff_BC,
  status := "BC top 5% only"
]
...
pA <- ggplot(
  ...
  geom_vline(
    xintercept = cutoff_BC,
    linetype = "dotted",
    color = "grey60",
    linewidth = 0.6
  ) +
  geom_hline(
    yintercept = cutoff_AK,
    linetype = "dotted",
    color = "grey60",
    linewidth = 0.6
  ) +
