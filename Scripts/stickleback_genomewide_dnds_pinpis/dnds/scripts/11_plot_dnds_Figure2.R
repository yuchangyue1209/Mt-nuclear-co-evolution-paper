...
mt_file <- "/mnt/spareHD_2/oxphos_codeml_ready/09_codeml_sites_models/codeml_sites_summary.merged.tsv"
outdir <- file.path(root, "09_figures/Figure2_complete_updated_large_fonts")
...
complex_panel <- function(metric) {
  ...
  g <- ggplot(d,aes(Complex,.data[[metric]],fill=group)) +
    ...
    theme(axis.text.x=element_text(size=AXIS_TEXT_SIZE,angle=0,hjust=.5),legend.position="top",legend.title=element_blank(),legend.text=element_text(size=23))
  list(plot=g,stats=st)
}
...
rowA <- make_row(A, c("mt","nu","ass"), pal_overall, lab_overall,
                 "A_overall")
rb <- lapply(c("dN","dS","omega_plot"), complex_panel)
rowB <- list(
  plot = rb[[1]]$plot | rb[[2]]$plot | rb[[3]]$plot,
  stats = do.call(rbind, lapply(rb, "[[", "stats"))
)

# New panel order: OXPHOS comparisons first, followed by RP and ARS comparisons.
# C and D below use data objects E and F from the data-preparation section.
# E and F below use data objects C and D, respectively.
rowC <- make_row(E, c("mt","nu_core","nu_noncore"), pal_core, lab_core,
                 "C_core")
rowD <- make_row(F, c("direct_n-mt","indirect_n-mt","non-n-mt"),
                 pal_kuster, lab_kuster, "D_Kuster", 1500)
rowE <- make_row(C, c("cyto-ribo","Nmt-ribo"), pal_rp, lab_rp,
                 "E_RP")
rowF <- make_row(D, c("cyto-ARS","Nmt-ARS"), pal_ars,
                 c(`cyto-ARS`="cyto-ARS", `Nmt-ARS`="Nmt-ARS"), "F_ARS")
...
for(tag in names(plots)) {
  stub <- c(
    A="Figure2A_mt_nu_assembly",
    B="Figure2B_complex",
    C="Figure2C_core_noncore",
    D="Figure2D_Kuster",
    E="Figure2E_RP",
    F="Figure2F_ARS"
  )[[tag]]
