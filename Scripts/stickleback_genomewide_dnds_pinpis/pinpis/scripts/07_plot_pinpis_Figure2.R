...
BORDER_LWD<-0.7; AXIS_TITLE_SIZE<-34; AXIS_TEXT_SIZE<-24; N_SIZE<-6.5; STAR_SIZE<-9
ROW_TAG_SIZE<-36; POINT_SIZE<-1.9; POINT_ALPHA<-0.62; set.seed(20260810)
...
panel_data<-list(
 ...
 C=list(data=dat[!is.na(core_plot_group)],var="core_plot_group",levels=c("mtOXPHOS","nu core","nu noncore"),labels=c(mtOXPHOS="mtOXPHOS",`nu core`="nu-core",`nu noncore`="nu-noncore"),pal=pal_core,panel="E_core_status"),
 D=list(data=dat[gene_source=="nuclear" & Kuster_class%in%c("direct_n-mt","indirect_n-mt","non-n-mt")],var="Kuster_class",levels=c("direct_n-mt","indirect_n-mt","non-n-mt"),labels=c(`direct_n-mt`="direct n-mt",`indirect_n-mt`="indirect n-mt",`non-n-mt`="non-n-mt"),pal=pal_kuster,panel="F_Kuster"),
 E=list(data=dat[plot_role%in%c("cyto-RP","Nmt-RP")],var="plot_role",levels=c("cyto-RP","Nmt-RP"),labels=c(`cyto-RP`="cyto-RP",`Nmt-RP`="Nmt-RP"),pal=pal_rp,panel="C_RP"),
 F=list(data=dat[plot_role%in%c("cyto-ARS","Nmt-ARS")],var="plot_role",levels=c("cyto-ARS","Nmt-ARS"),labels=c(`cyto-ARS`="cyto-ARS",`Nmt-ARS`="Nmt-ARS"),pal=pal_ars,panel="D_ARS"))
...
make_simple<-function(spec,metric,point_cap=Inf){
 ...
 # Hide points above the display cap so they do not spill into adjacent panels.
 # The full data in d are still used for boxplots and statistical tests.
 draw<-draw[get(value)<=cap]
...
make_complex<-function(metric){
 ...
 g<-ggplot(d,aes(complex,.data[[value]],fill=group))+
  ...
  labs(x=NULL,y=metric_lab[[metric]])+base_theme+theme(axis.text.x=element_text(size=AXIS_TEXT_SIZE,angle=0,hjust=.5),
   legend.position="top",legend.title=element_blank(),legend.text=element_text(size=23))
...
make_row<-function(tag){spec<-panel_data[[tag]];cap<-if(spec$panel=="F_Kuster")1500 else Inf
...
tag_row<-function(p,tag)p+plot_annotation(tag_levels=list(c(tag,"","")))&
 theme(plot.tag=element_text(size=ROW_TAG_SIZE,face="bold"))
rows<-Map(tag_row,rows,names(rows))
stubs<-c(A="Figure2A_pi_mt_nu_assembly",B="Figure2B_pi_complex",C="Figure2C_pi_core_noncore",D="Figure2D_pi_Kuster",E="Figure2E_pi_RP",F="Figure2F_pi_ARS")
...
combined<-wrap_plots(rows,ncol=1,heights=c(1,1.08,1,1,1,1))+plot_annotation(
 caption="Dotted lines indicate genome-wide nuclear medians. Stars show within-panel BH-adjusted Wilcoxon tests. For display only, panel D is capped at the 99th percentile; all observations were retained in statistical analyses.")
